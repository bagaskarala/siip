            ...
            <table class="awn-table">
                ...
                <tbody>
                    ...
                    <?= ($i & 1) ? '<tr class="zebra">' : '<tr>'; ?>
                        ...
                        <td><?= anchor("admin/admin/worksheet/edit/$worksheet->worksheet_id", 'Edit', ['class' => 'btn btn-warning']) ?></td>
                        <td>
                            <?php if ($worksheet->worksheet_status > 0) {
                                echo "";
                            } else {
                                ?>
                                <?= form_open("admin/worksheet/action/$worksheet->worksheet_id/1") ?>
                                    <?= form_hidden('worksheet_id', $worksheet->worksheet_id) ?>
                                    <?= form_button(['type' => 'submit', 'content' => 'Approve', 'class' => 'btn-success']) ?>
                                <?= form_close() ?>

                                <?= form_open("admin/worksheet/action/$worksheet->worksheet_id/2") ?>
                                    <?= form_hidden('worksheet_id', $worksheet->worksheet_id) ?>
